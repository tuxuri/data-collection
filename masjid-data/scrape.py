...
fieldnames = [
    "no", "code", "kategori", "nama", "alamat", "negeri", "daerah", "no_tel", "no_fax", "tarikh_dibina", "lat", "long",
    "sejarah", "binaan", "kemudahan", "gambar",
]

logging.basicConfig(filename='masjid-data/app.log', filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.INFO)

def main():
    Path(DOWNLOAD_DIR).mkdir(parents=True, exist_ok=True)

    history = {}
    if Path(HISTORY_CONFIG).is_file():
        with open(HISTORY_CONFIG) as f:
            history = json.load(f)

    last_page = get_last_page(fetch(history, ENDPOINT))
    if not last_page:
        logging.error("Failed to get last page")
        return
    ...
    with open(STORES, mode='w', newline='', encoding="utf-8") as csvfile:
        ...
        for i in range(last_page):
            ...
            if i + 1 in [10, 38, 51, 59, 190, 197, 334, 445, 469, 483, 516, 517, 608, 613, 614, 615, 616, 640]:
                logging.info(f"Skipping page {i + 1}")
                continue
            url = f"{ENDPOINT}{i + 1}"
...
def extract_data(data, history):
    ...
    for info_div in info_divs:
        image_src = info_div.select_one("div.row.col-12 div.col-md-3 img")[
            "src"] if info_div.select_one("div.row.col-12 div.col-md-3 img") else None
        ...
        next_scraping_link = info_div.select_one("div.row.col-12 div.col-md-5 a")[
            "href"] if info_div.select_one("div.row.col-12 div.col-md-5 a") else None
        ...
        if next_scraping_link:
            ...
            logging.warning("Some information not found for this set.")
